...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // 세션에서 사용자 ID 가져오기
    $user_id = $_SESSION['id']; // 사용자 ID가 세션에 저장되어 있다고 가정
    $password = $_POST['pw'];

    // 비밀번호 확인 쿼리
    $sql = "SELECT * FROM user WHERE id = '$user_id' AND pw = '$password'";
    $result = $conn->query($sql);

    if ($result && $result->num_rows > 0) {
        // 비밀번호가 일치하면 사용자 삭제
        $delete_sql = "DELETE FROM user WHERE id = '$user_id' AND pw='$password'";
                    ...
                    <!-- 회원 탈퇴 폼 -->
                        ...
                        <form action = "user_delete.php" class="form-signin" style="max-width:500px;  margin: 0 auto;" th:action="@{/account/myinfo}" method="post" th:object="${member}">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="password" name = "pw" placeholder = '비밀번호 입력' class="form-control" id = "password_1">
                            ...
                            <div class="mb-3">
                                ...
                                <input type = "password" name = "pw2" placeholder = '비밀번호 재입력' class = "form-control" id = "password_2" >
